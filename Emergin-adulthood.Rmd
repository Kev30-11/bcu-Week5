...
We can get that coloum with select!
...
```{r}
adulthood_codebook_raw %>%
  filter(str_detect(question_text, "social events"))
```
...
```{r}
idea_responses1 <-adulthood_data_raw %>%
  select(ResponseId, starts_with("SocMedia_"))

idea_responses_long1 <-idea_responses1 %>%
...
```{r}
idea_questions1 <- adulthood_codebook_raw %>%
  select(variable_name, question_text) %>%
...
```{r}
idea_responses_long1 %>%
  left_join(idea_questions1,
            by = c("name" = "variable_name")) %>%
  count(question_text,value_label) %>%
  ggplot()+
  aes(x=n,
